...
9. Merge a pull request only when the user explicitly requests that exact merge. With that explicit authorization, Codex may verify and merge the pull request without asking again.
10. Before merging, confirm that the pull request is mergeable and that required checks pass. Never bypass branch protection or force a merge.
