...
import { resetJapaneseScoringSuggestion } from "@/lib/game/gameService";
import {
...
export async function POST(request: NextRequest, context: { params: Promise<{ gameId: string }> }) {
  try {
    ...
    const game = await resetJapaneseScoringSuggestion(gameId, playerKey, body.expectedRevision as number);
