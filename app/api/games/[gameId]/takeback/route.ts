...
import { requestTakeback, respondToTakeback } from "@/lib/game/gameService";
import {
...
export async function POST(
  ...
  try {
    ...
    const body = await readGameMutationJson(request, [
      ...
      ["action", "accept", "expectedVersion"],
    ]);
    if (!Number.isSafeInteger(body.expectedVersion) || Number(body.expectedVersion) < 0) {
      throw invalidGameMutationRequest();
    }
    const game = body.action === "request" && body.accept === undefined
      ? await requestTakeback(gameId, playerKey, body.expectedVersion as number)
      : body.action === "respond" && typeof body.accept === "boolean"
        ? await respondToTakeback(
            gameId,
            playerKey,
