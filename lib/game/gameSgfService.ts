...
type ExportGameRow = QueryResultRow & {
  ...
  bot_rating_mode: "unrated" | "calibrated-v1" | "browser-v1" | null;
  bot_profile_id: string | null;
  bot_engine_family: string | null;
  bot_model_version: string | null;
  bot_config_version: string | null;
  browser_model_contract_version: string | null;
  browser_model_version: string | null;
  browser_model_sha256: string | null;
};
...
function noResultReason(outcomeKind: string | undefined): Extract<SgfTerminalResult, { kind: "no-result" }>["reason"] {
  switch (outcomeKind) {
    ...
    case "unresolved": return "unresolved-after-participation";
...
async function loadSgfInput(
  ...
  const gameResult = await client.query<ExportGameRow>(
    `SELECT g.id,g.board_size,g.black_player_key,g.white_player_key,g.winner_key,
            ...
            browser_binding.model_contract_version AS browser_model_contract_version,
            browser_binding.model_version AS browser_model_version,
            browser_binding.model_sha256 AS browser_model_sha256
       FROM games g
  ...
  const botDisclosure = game.bot_player_key && game.bot_color ? game.bot_rating_mode === "browser-v1"
    ? {
        provider: "GoStone browser AI",
        model: game.browser_model_version ?? "not-recorded",
        version: game.browser_model_contract_version && game.browser_model_sha256
          ? `${game.browser_model_contract_version}/${game.browser_model_sha256}`
          : "browser-v1",
      }
    : {
        provider: game.bot_engine_family ?? "calibrated bot",
