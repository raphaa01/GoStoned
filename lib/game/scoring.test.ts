...
test("policy dispatch preserves immediate and agreement scoring boundaries", () => {
  ...
  const current = resolveRulesPolicy(CHINESE_2002_RULES_PROFILE);
...
test("Chinese profiles use positional superko and Japanese uses simple ko", () => {
  const priorHashes = new Set(["seen"]);
  for (const profile of [LEGACY_IMMEDIATE_AREA_PROFILE, CHINESE_2002_RULES_PROFILE]) {
  ...
  const japanese = resolveRulesPolicy(DEFAULT_RULES_PROFILE);
  assert.equal(isRepeatedPositionForbidden(japanese, "seen", ["seen", "other"]), true);
  assert.equal(isRepeatedPositionForbidden(japanese, "new", ["seen", "other"]), false);
});
...
test("Chinese dispute resumption gives the first move to the player claiming death", () => {
  ...
  const japanese = resolveRulesPolicy(DEFAULT_RULES_PROFILE);
  assert.equal(resumeTurnForPolicy(japanese, "black", "dead"), "white");
  assert.equal(resumeTurnForPolicy(japanese, "black", "alive"), "white");
  assert.throws(
