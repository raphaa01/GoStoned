...
const sections: LocalizedSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      text(
        "Puzzles, bots, and analysis: puzzle selections, attempts, progress, solution status, and timestamps are stored. If you request a KataGo review or play a bot, the game identifier, rules, moves, and generated analysis or bot move are processed by our separate KataGo worker. Legal basis: Article 6(1)(b) GDPR.",
        "Puzzles, Bots und Analyse: Puzzleauswahl, Versuche, Fortschritt, Lösungsstatus und Zeitpunkte werden gespeichert. Wenn du eine KataGo-Analyse anforderst oder gegen einen Bot spielst, verarbeitet unser separater KataGo-Worker Partiekennung, Regeln, Züge und die erzeugte Analyse beziehungsweise den Bot-Zug. Rechtsgrundlage: Art. 6 Abs. 1 lit. b DSGVO.",
        "Problèmes, robots et analyse : les sélections, tentatives, progrès, état de résolution et horodatages sont conservés. Si vous demandez une analyse KataGo ou jouez contre un robot, notre worker KataGo séparé traite l’identifiant de partie, les règles, les coups et l’analyse ou le coup généré. Base juridique : article 6, paragraphe 1, point b) du RGPD.",
        "Problemas, bots y análisis: se guardan selección, intentos, progreso, estado de resolución y marcas de tiempo. Si solicitas un análisis de KataGo o juegas contra un bot, nuestro worker independiente procesa identificador de partida, reglas, movimientos y el análisis o movimiento generado. Base jurídica: artículo 6.1.b del RGPD.",
        "题目、机器人和分析：我们保存题目选择、尝试次数、进度、解题状态和时间戳。如果您请求 KataGo 复盘或与机器人对局，独立的 KataGo 工作进程会处理对局标识符、规则、落子以及生成的分析或机器人着法。法律依据：GDPR 第6条第1款(b)项。",
        "問題、ボット、解析：問題の選択、試行、進行状況、正解状態、時刻を保存します。KataGo 解析を依頼した場合やボットと対局する場合、別の KataGo ワーカーが対局 ID、ルール、着手、生成された解析またはボットの着手を処理します。法的根拠：GDPR 第6条1項(b)。",
        "문제, 봇 및 분석: 문제 선택, 시도, 진행 상황, 해결 여부 및 시각을 저장합니다. KataGo 분석을 요청하거나 봇과 대국하면 별도 KataGo 워커가 대국 식별자, 규칙, 수순 및 생성된 분석이나 봇 착점을 처리합니다. 법적 근거: GDPR 제6조 제1항 (b).",
      ),
    ],
  },
  {
...
const processors = {
  ...
  entries: [
    ...
    {
      name: "Modal Labs, Inc.",
      purpose: text("Isolated cloud execution of the KataGo worker for game analysis, bot moves, and puzzle generation. Relevant game positions and moves are processed in worker memory; application and container logs are retained according to the Modal plan.", "Isolierte Cloud-Ausführung des KataGo-Workers für Partieanalyse, Bot-Züge und Puzzleerzeugung. Relevante Partiestellungen und Züge werden im Arbeitsspeicher des Workers verarbeitet; Anwendungs- und Containerprotokolle werden abhängig vom Modal-Tarif gespeichert.", "Exécution cloud isolée du worker KataGo pour l’analyse, les coups de robots et la génération de problèmes. Les positions et coups concernés sont traités en mémoire ; les journaux d’application et de conteneur sont conservés selon l’offre Modal.", "Ejecución aislada en la nube del worker KataGo para análisis, movimientos de bots y generación de problemas. Las posiciones y movimientos relevantes se tratan en memoria; los registros se conservan según el plan Modal.", "在隔离云环境中运行 KataGo 工作进程，用于对局分析、机器人落子和题目生成。相关棋局状态与落子在工作进程内存中处理；应用和容器日志依 Modal 方案保存。", "対局解析、ボット着手、問題生成のための KataGo ワーカーを隔離されたクラウドで実行します。関連する局面と着手はワーカーのメモリで処理され、アプリ・コンテナログは Modal のプランに応じて保存されます。", "대국 분석, 봇 착점 및 문제 생성을 위한 KataGo 워커를 격리된 클라우드에서 실행합니다. 관련 국면과 수순은 워커 메모리에서 처리되며 애플리케이션 및 컨테이너 로그는 Modal 요금제에 따라 보관됩니다."),
